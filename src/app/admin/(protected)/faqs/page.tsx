...
const defaultFAQs: FAQ[] = [
  ...
  { id: "7", question: "How much does Printer rental cost in Abu Dhabi?", answer: "Our Printer rental in Abu Dhabi StarIconts from AED 250/month for basic Printers, with enterprise copiers from AED 500-2000/month. All plans include free toner and maintenance.", pageSlug: "Printer-rental-abu-dhabi", isActive: true, sortOrder: 1 },
  ...
  { id: "9", question: "How much does photocopier rental cost in Sharjah?", answer: "Our photocopier rental in Sharjah StarIconts from AED 250/month. We offer competitive rates with zero deposit and free toner included in all plans.", pageSlug: "photocopier-rental-sharjah", isActive: true, sortOrder: 1 },
...
export default function AdminFAQs() {
  ...
  return (
    <div className="min-h-screen bg-[#071325]">
      ...
      {showModal && (
        <FAQModal faq={editingFAQ} pageOptions={pageOptions} onSave={handleSave} onCloseIcon={() => { setShowModal(false); setEditingFAQ(null); }} />
      )}
    </div>
  );
}

function FAQModal({ faq, pageOptions, onSave, onCloseIcon }: { faq: FAQ | null; pageOptions: PageOption[]; onSave: (f: FAQ) => void; onCloseIcon: () => void }) {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-6">
      <div className="glass-card rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={onCloseIcon} className="text-slate-400 hover:text-white"><span className="material-symbols-outlined">CloseIcon</span></button>
        </div>
