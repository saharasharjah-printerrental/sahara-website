...
const initialSupplies: Supply[] = [
  ...
  { id: "4", name: "Canon C5045/5051/5250/5255 Toner Premium Magenta", brand: "Canon", category: "Toner", compatibleModels: "C5045, C5051, C5250, C5255", color: "Magenta", yield: "25,000 pages", price: "Contact for Pricing", stock: 50, image: "", altText: "", imageWidth: 800, imageHeight: 800, isActive: true },
  { id: "5", name: "Canon C5035/5040/5235/5240 Toner Premium Black", brand: "Canon", category: "Toner", compatibleModels: "C5035, C5040, C5235, C5240", color: "Black", yield: "23,000 pages", price: "Contact for Pricing", stock: 30, image: "", altText: "", imageWidth: 800, imageHeight: 800, isActive: true },
  { id: "6", name: "Canon C5035/5040/5235/5240 Toner Premium Cyan", brand: "Canon", category: "Toner", compatibleModels: "C5035, C5040, C5235, C5240", color: "Cyan", yield: "23,000 pages", price: "Contact for Pricing", stock: 30, image: "", altText: "", imageWidth: 800, imageHeight: 800, isActive: true },
  { id: "7", name: "Canon C5035/5040/5235/5240 Toner Premium Yellow", brand: "Canon", category: "Toner", compatibleModels: "C5035, C5040, C5235, C5240", color: "Yellow", yield: "23,000 pages", price: "Contact for Pricing", stock: 30, image: "", altText: "", imageWidth: 800, imageHeight: 800, isActive: true },
  { id: "8", name: "Canon C5035/5040/5235/5240 Toner Premium Magenta", brand: "Canon", category: "Toner", compatibleModels: "C5035, C5040, C5235, C5240", color: "Magenta", yield: "23,000 pages", price: "Contact for Pricing", stock: 30, image: "", altText: "", imageWidth: 800, imageHeight: 800, isActive: true },
  { id: "9", name: "Compatible C-EXV51 Toner Cartridge", brand: "Canon", category: "Toner", compatibleModels: "imageRUNNER Advance C5535, C5540, C5550, C5560", color: "Black", yield: "36,000 pages", price: "Contact for Pricing", stock: 25, image: "", altText: "", imageWidth: 800, imageHeight: 800, isActive: true },
  { id: "10", name: "Long Life OPC Drum for Canon IR ADVANCE C5535 C5540 C5550 C5560", brand: "Canon", category: "Drum", compatibleModels: "C5535, C5540, C5550, C5560, IRC 5535, 5540, 5550", yield: "150,000 pages", price: "Contact for Pricing", stock: 15, image: "", altText: "", imageWidth: 800, imageHeight: 800, isActive: true },
...
function mapSupply(s: any): Supply {
  return {
    ...
    name: s.name,
    brand: s.brand || '',
    category: s.category as Supply['category'],
    compatibleModels: s.compatible_models || s.compatibleModels || '',
    color: s.color || '',
    yield: s.yield || '',
    price: s.price || '',
    stock: s.stock ?? 0,
    ...
    imageWidth: s.image_width || s.imageWidth || 800,
    imageHeight: s.image_height || s.imageHeight || 800,
...
export default function AdminSupplies() {
  ...
  const fetchSupplies = async () => {
    try {
      ...
      if (data.supplies && data.supplies.length > 0) {
        ...
        setSupplies(mapped);
        localStorage.setItem("sahara_supplies", JSON.stringify(mapped));
      } else if (data.error) {
        const stored = localStorage.getItem("sahara_supplies");
        setSupplies(stored ? JSON.parse(stored) : initialSupplies);
      } else {
        const stored = localStorage.getItem("sahara_supplies");
        setSupplies(stored ? JSON.parse(stored) : initialSupplies);
      }
  ...
  const handleToggleActive = async (id: string) => {
    ...
    if (!supply) return;
    const updated = supplies.map(s => s.id === id ? { ...s, isActive: !s.isActive } : s);
    setSupplies(updated);
      ...
      <main className="pt-8 pb-16 px-8 ml-64">
        <div className="max-w-7xl mx-auto">
            ...
            {["all", "active", "inactive"].map((f) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                  filter === f ? "bg-[#f5be53] text-[#412d00]" : "bg-[#101c2e] text-slate-400 hover:text-white"
                }`}
              >
          ...
          <div className="glass-card rounded-2xl overflow-hidden">
            {loading ? (
              <div className="p-8 text-center text-slate-400">Loading supplies...</div>
            ) : (
                      ...
                      <tr key={supply.id} className="border-b border-white/5">
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            <div className="w-12 h-12 rounded-lg bg-[#101c2e] flex items-center justify-center overflow-hidden">
                              {supply.image ? (
                                <img src={supply.image} alt={supply.altText || supply.name} className="w-full h-full object-contain" />
                              ) : supply.color ? (
                        ...
                        <td className="p-4">
                          <button
                            ...
                            className={`px-3 py-1 rounded-full text-xs font-medium ${
                              supply.isActive ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"
                            }`}
                          >
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={form.brand} onChange={(e) => setForm({ ...form, brand: e.target.value })} className="w-full bg-[#101c2e] border border-white/10 rounded-xl py-3 px-4 text-white">
                {brands.map((b) => <option key={b}>{b}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">Category</label>
              <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value as any })} className="w-full bg-[#101c2e] border border-white/10 rounded-xl py-3 px-4 text-white">
          ...
          {form.category === "Toner" && (
            <div>
              ...
              <select value={form.color || ""} onChange={(e) => setForm({ ...form, color: e.target.value })} className="w-full bg-[#101c2e] border border-white/10 rounded-xl py-3 px-4 text-white">
          ...
          <div>
            ...
            <input type="text" value={form.compatibleModels} onChange={(e) => setForm({ ...form, compatibleModels: e.target.value })} className="w-full bg-[#101c2e] border border-white/10 rounded-xl py-3 px-4 text-white" placeholder="e.g., C5045, C5051, C5250" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">Page Yield</label>
              <input type="text" value={form.yield} onChange={(e) => setForm({ ...form, yield: e.target.value })} className="w-full bg-[#101c2e] border border-white/10 rounded-xl py-3 px-4 text-white" placeholder="e.g., 25,000 pages" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">Stock Quantity</label>
              <input type="number" value={form.stock} onChange={(e) => setForm({ ...form, stock: parseInt(e.target.value) || 0 })} className="w-full bg-[#101c2e] border border-white/10 rounded-xl py-3 px-4 text-white" />
            </div>
          </div>
          <div>
          ...
          <div className="flex gap-4 pt-4">
            <button onClick={onClose} className="flex-1 py-3 rounded-xl bg-[#101c2e] text-slate-300 hover:text-white transition-colors">Cancel</button>
            <button onClick={() => onSave(form)} className="flex-1 bg-gradient-to-r from-[#f5be53] to-[#c8962e] text-[#412d00] py-3 rounded-xl font-bold hover:scale-[1.02] transition-transform">Save Supply</button>
          </div>
        </div>
      </div>
    </div>
